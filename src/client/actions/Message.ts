...
export class Message {
  ...
  private validateTemplateComponents(components: TemplateComponent[]): void {
    for (const component of components) {
      ...
      if (
        ...
        throw new WhatsAppApiException(
          `Parameters are required for ${component.type} component`,
          0
        );
      }
      ...
      for (const param of component.parameters) {
        ...
        switch (param.type) {
          case "text":
            if (!param.text) {
              throw new WhatsAppApiException(
                "Text value is required for text parameter",
                0
              );
            }
            break;
          case "currency":
            if (!param.currency) {
              throw new WhatsAppApiException(
                "Currency object is required for currency parameter",
                0
              );
            }
            if (
              ...
              throw new WhatsAppApiException(
                "Currency parameter must include fallback_value, code, and amount_1000",
                0
              );
            }
            break;
          case "date_time":
            if (!param.date_time || !param.date_time.fallback_value) {
              throw new WhatsAppApiException(
                "Date time parameter must include fallback_value",
                0
              );
            }
            break;
          case "image":
            if (!param.image || !param.image.link) {
              throw new WhatsAppApiException(
                "Image parameter must include a link",
                0
              );
            }
            if (!this.isValidUrl(param.image.link)) {
              throw new WhatsAppApiException(
                `Invalid URL format for image: ${param.image.link}`,
                0
              );
            }
            break;
          case "document":
            if (!param.document || !param.document.link) {
              throw new WhatsAppApiException(
                "Document parameter must include a link",
                0
              );
            }
            if (!this.isValidUrl(param.document.link)) {
              throw new WhatsAppApiException(
                `Invalid URL format for document: ${param.document.link}`,
                0
              );
            }
            break;
          case "video":
            if (!param.video || !param.video.link) {
              throw new WhatsAppApiException(
                "Video parameter must include a link",
                0
              );
            }
            if (!this.isValidUrl(param.video.link)) {
              throw new WhatsAppApiException(
                `Invalid URL format for video: ${param.video.link}`,
                0
              );
            }
            break;
          case "payload":
            if (!param.payload) {
              throw new WhatsAppApiException(
                "Payload value is required for payload parameter",
                0
              );
            }
            break;
          case "action":
            if (!param.action) {
              throw new WhatsAppApiException(
                "Action object is required for action parameter",
                0
              );
            }
            ...
            if (
              ...
              throw new WhatsAppApiException(
                "thumbnail_product_retailer_id is required for CATALOG button",
                0
              );
            }
            break;
  ...
  private buildMessageBody(payload: MessagePayload): MessageBodyPayload {
    ...
    if (payload.template) {
      ...
      if (!hasComponents) {
        throw new WhatsAppApiException("Components are required for embeds", 0);
      }
      ...
      if (hasValidHeaderFile) {
        ...
        messageBody.interactive = {
          ...
          header: {
            type: file.type as "image" | "video" | "document",
        ...
        switch (interactiveType) {
          ...
          case "list":
            messageBody.interactive.action = {
              ...
              button: (payload.components[0] as unknown as ListBuilder)
                .buttonText,
            };
            break;
          default:
