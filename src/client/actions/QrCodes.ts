import type { CreateQrCodeParams, QrCodeListResponse, QrCodeRecord, QrCodeSuccessResponse } from "../../types"
import { WhatsAppApiException } from "../../errors/Messages"
import type { Client } from "../Client"

const MAX_PREFILLED_MESSAGE_LENGTH = 140

/**
 * Manages click-to-chat QR codes and short links for the phone number, via the
 * `message_qrdls` endpoint.
 */
export class QrCodeManager {
  private client: Client

  constructor(client: Client) {
    this.client = client
  }

  private validatePrefilledMessage(message: string): void {
    if (!message) {
      throw new WhatsAppApiException("prefilledMessage is required", 0)
    }

    if (message.length > MAX_PREFILLED_MESSAGE_LENGTH) {
      throw new WhatsAppApiException(
        `prefilledMessage must be ${MAX_PREFILLED_MESSAGE_LENGTH} characters or less`,
        0,
      )
    }
  }

  /**
   * Creates a new QR code / short link that opens a chat pre-filled with a message.
   * @param params The pre-filled message and optional QR image format
   * @returns The created QR code record
   */
  async create(params: CreateQrCodeParams): Promise<QrCodeRecord> {
    this.validatePrefilledMessage(params.prefilledMessage)

    return this.client.makeApiRequest<QrCodeRecord>("message_qrdls", "POST", {
      prefilled_message: params.prefilledMessage,
      ...(params.generateQrImage ? { generate_qr_image: params.generateQrImage } : {}),
    })
  }

  /**
   * Lists every QR code / short link created for the phone number.
   * @returns The list of QR codes
   */
  async list(): Promise<QrCodeListResponse> {
    return this.client.makeApiRequest<QrCodeListResponse>("message_qrdls", "GET")
  }

  /**
   * Gets a single QR code by its code ID.
   * @param codeId QR code ID
   * @returns The QR code record
   */
  async get(codeId: string): Promise<QrCodeRecord> {
    if (!codeId) {
      throw new WhatsAppApiException("codeId is required", 0)
    }

    return this.client.makeApiRequest<QrCodeRecord>(`message_qrdls/${codeId}`, "GET")
  }

  /**
   * Updates the pre-filled message of an existing QR code. The code and link stay the same.
   * @param codeId QR code ID
   * @param prefilledMessage New pre-filled message
   * @returns The updated QR code record
   */
  async update(codeId: string, prefilledMessage: string): Promise<QrCodeRecord> {
    if (!codeId) {
      throw new WhatsAppApiException("codeId is required", 0)
    }

    this.validatePrefilledMessage(prefilledMessage)

    return this.client.makeApiRequest<QrCodeRecord>("message_qrdls", "POST", {
      code: codeId,
      prefilled_message: prefilledMessage,
    })
  }

  /**
   * Deletes a QR code / short link.
   * @param codeId QR code ID
   * @returns API response
   */
  async delete(codeId: string): Promise<QrCodeSuccessResponse> {
    if (!codeId) {
      throw new WhatsAppApiException("codeId is required", 0)
    }

    return this.client.makeApiRequest<QrCodeSuccessResponse>(`message_qrdls/${codeId}`, "DELETE")
  }
}
